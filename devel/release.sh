...
main() {
  ...
  version="$1"; shift
  if [[ "$version" == "" ]]; then
    printf "What version do you want to release (current latest is `git describe --tags --abbrev=0 2>/dev/null`)? "
    read version
  fi
  ...
  echo "Don't forget to upload `find . -name 'substreams-sink-files-*.spkg' | tr -d "\n"` to GitHub release page and to publish the release!"
}
